...
public class Tweaker {
    ...
    private void updateConfig(Configuration configuration, boolean load) {
        ...
        if (load) NAUSEAEFFECT = prop.getBoolean();
        else prop.setValue(NAUSEAEFFECT);

        prop = configuration.get("General", "respawnButton", true);
        if (load) RESPAWNBUTTON = prop.getBoolean();
        else prop.setValue(RESPAWNBUTTON);

        prop = configuration.get("General", "armPosition", false);
        if (load) ARMPOSITION = prop.getBoolean();
        else prop.setValue(ARMPOSITION);

        prop = configuration.get("General", "instantWorldSwitching", true);
        if (load) INSTANTWORLDSWITCHING = prop.getBoolean();
        else prop.setValue(INSTANTWORLDSWITCHING);

        prop = configuration.get("General", "windowedFullscreen", false);
        if (load) WINDOWEDFULLSCREEN = prop.getBoolean();
        else prop.setValue(WINDOWEDFULLSCREEN);

        prop = configuration.get("General", "mouseDelayFix", false);
        if (load) MOUSEDELAYFIX = prop.getBoolean();
        else prop.setValue(MOUSEDELAYFIX);

        prop = configuration.get("General", "noCloseMyChat", false);
        if (load) NOCLOSEMYCHAT = prop.getBoolean();
        else prop.setValue(NOCLOSEMYCHAT);

        prop = configuration.get("General", "scoreboardCrash", true);
        if (load) SCOREBOARDCRASH = prop.getBoolean();
        else prop.setValue(SCOREBOARDCRASH);

        prop = configuration.get("General", "asyncScreenshots", false);
        if (load) ASYNCSCREENSHOTS = prop.getBoolean();
        else prop.setValue(ASYNCSCREENSHOTS);

        prop = configuration.get("General", "fullbright", false);
        if (load) FULLBRIGHT = prop.getBoolean();
        else prop.setValue(FULLBRIGHT);

        prop = configuration.get("General", "voidFlickerFix", false);
        if (load) VOIDFLICKERFIX = prop.getBoolean();
        else prop.setValue(VOIDFLICKERFIX);

        prop = configuration.get("General", "inventoryPotionEffectPosition", false);
        if (load) INVENTORYPOTIONPOSITION = prop.getBoolean();
        else prop.setValue(INVENTORYPOTIONPOSITION);

        prop = configuration.get("General", "raytraceBlocks", false);
        if (load) RAYTRACEBLOCKS = prop.getBoolean();
        else prop.setValue(RAYTRACEBLOCKS);
    }
}
